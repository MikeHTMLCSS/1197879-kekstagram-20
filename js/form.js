...
window.makePicture = function (filterType) {
  ...
  formInput.addEventListener('submit', function () {
    ...
    for (var i = 0; i < tags.length; i++) {
      var hashTag = tags[i].split('');
      if (hashTag[0] !== '#' || hashTag.length < 2 || hashTag.length > 20) {
        valid = false;
      }
      for (var j = 0; j < hashTag.length; j++) {
        if (hashTag[j] === '!' || hashTag[j] === '@' || hashTag[j] === '№' || hashTag[j] === '$' || hashTag[j] === '%' || hashTag[j] === '^' || hashTag[j] === '&' || hashTag[j] === '+' || hashTag[j] === '-' || hashTag[j] === '*' || hashTag[j] === '/' || (hashTag[j] === '#' && j !== 0)) {
          valid = false;
        }
      }
    }
